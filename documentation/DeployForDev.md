# Развертывание проекта для разработки (временно единственное решение, пока не появится докер образ)
...
* <a href="#tts_engine_install"> Установка TTS-движков и сопутствующих утилит </a>
  - <a href="#sox_install"> Установка и настройка Sox </a>
  - <a href="#google_engine_install"> Установка и настройка Google Text-To-Speech </a>
...
<h2 name = "tts_engine_install">Установка TTS-движков и сопутствующих утилит</h2> <a href="#context">(к содержанию)</a>

-----

<h3 name = "sox_install">Установка и настройка Sox</h3>

Sox - консольный аудиоредактор. Здесь используется для преобразования звуковых файлов в формат, поддерживаемый asterisk.
По умолчанию он уже лежит в корне репозитория, но при необходимости есть возможность установить другую версию утилиты.
Для этого достаточно создать системную переменную `SOX_PATH` с указанием на sox.exe,
...
<h3 name = "google_engine_install">Установка и настройка Google Text-To-Speech</h3>
