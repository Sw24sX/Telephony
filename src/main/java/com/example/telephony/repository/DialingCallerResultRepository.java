...
public interface DialingCallerResultRepository extends JpaRepository<DialingCallerResult, Long> {
    ...
    @Query("select d from DialingCallerResult d where d.dialing.id = ?1 and d.status = ?2 order by d.created")
    List<DialingCallerResult> getDialingCallerResultByDialingIdAndStatusOrderByCreated(Long dialingId, DialCallerStatus status);
    ...
    List<DialingCallerResult> getDialingResultsOrder(Long dialingId);
}